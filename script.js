...
const addTaskToList = () => {
 ...
 if(taskTXT !== ""){ //create the element/task
    ...
    taskList.appendChild(newTask); //adding all the elements to list
    ...
    editBtn2.addEventListener("click", editTask);
       ...
       emptyTXT.textContent = `Enter the task first!`; //if no task was typed and there is add attempt
    }
...
const editTask = (e) => {
    popUpWindow.classList.remove('hidden'); //ok
    backdrop.classList.remove('hidden'); 
    inputPopUp.value = e.target.closest('.tools').previousElementSibling.textContent; //tekst edytowanego zadanie w inpucie
    thisTask = e;
...
const acceptChange = () => {
    ...
    if(inputPopUp.value == ""){
        ...
        thisTask.target.closest('.tools').previousElementSibling.textContent = inputPopUp.value;
